...
- 两个仓库的发行版均已创建（预发布，同一组附件）：GitHub `387662208`、Gitee `1140399`，各含 `ncpu-timetable-1.0.2-universal.apk` 与 `app-arm64-v8a-release.apk`。

Changed:
- `master` 更新为 `9265922` 并同步到 Gitee 与 GitHub；`knowledge/release.md` 记录本版产物哈希、验证清单与分发状态。
...
- `CONFIRMED` 两个平台的通用包均实际下载比对：GitHub 40,526,034 B / 4.1 s、Gitee 40,526,034 B / 31.5 s，SHA-256 都与本地一致；两边发行版说明均为 440 汉字 / 0 U+FFFD。
- `CONFIRMED` Gitee 发行版初次尝试因本机只有 10 位 git 口令（API 报 `Access token does not exist`）而 `BLOCKED`，由用户提供令牌后补建，用毕需撤销。另记录双推不保证两边都成功，必须逐个读回。
...
Changed:
- Gitee 发行版 `1140075` 的说明去掉两行 SHA-256 校验值，改为测试者视角的结构：下载哪个文件、安装、用法、已知限制、反馈要点。标题保持 `v1.0.1 测试版`。
- 理由：收件人是普通测试者，长十六进制串对他们只是噪音；校验值仍保存在 `knowledge/release.md`，后续发布可直接取用。
- 改动只涉及发行版说明文本，未重新上传 APK，下载地址不变。

Validation:
- `CONFIRMED` 用 UTF-8 文件 + Python `PATCH /releases/{id}` 写入（沿用编码陷阱的正确做法），读回说明 283 个汉字、U+FFFD 为 0、与本地期望文本逐字一致，且不再含 `SHA-256`；四个附件完好。
...
Changed:
- 从 `README.md` 与 `knowledge/`（README、current_state、changelog、tasks、android_setup、android_agent_prompt）移除项目归属与跨知识库同步关系的公开表述，只保留「本目录是独立 Git 仓库与独立知识库、只记录本 App」。提交 `cbc0f20`。
- 该表述此前已随基线提交 `5509a85` 推送，且 `v1.0.1` 的发行版源码压缩包是 tag 快照、仍会带上它。因此删除旧发行版 `1140059`，删除并重建 tag `v1.0.1` 到清理后的提交，重新创建发行版 `1140075` 并重传两个 APK；**tag 名称与下载地址不变**。

Validation:
- `CONFIRMED` 工作区全量搜索该表述与相关路径：0 处命中；tag `v1.0.1` → `cbc0f20`（`git rev-parse v1.0.1^{commit}`）。
- `CONFIRMED` 下载重建后的源码包 `v1.0.1.zip`（686,693 B / 500 文件）逐文件搜索，确认不再包含该表述。
- `CONFIRMED` 重新下载通用包 40,476,186 B，SHA-256 `0b674d3b…f242694` 与本地构建产物一致；两个 APK 内部经检查本来就不含该文本（仅源码文档涉及）。临时文件已删除。

Note:
- **提交历史仍保留旧文本**（`git log -S` 可查到 `5509a85`）。彻底清除需重写历史并强推，属高风险操作、需单独授权，本轮未执行。
- 该保密要求已记入本地记忆（不落仓库），后续新增文档与提交信息不再带出。

## 2026-09-12 - Agent（TASK-050 发布 v1.0.1 测试版）

Published:
- Gitee 发行版 `1140059`（tag `v1.0.1`，预发布）创建完成，附 `ncpu-timetable-1.0.1-universal.apk`（40,476,186 B，通用包）与 `app-arm64-v8a-release.apk`（22,184,832 B）。说明含两个包的 SHA-256、安装提示与已知限制（该说明随后于同日改写为面向测试者的版本，不再含校验值）。
