...
- [ ] TASK-060 发布 v1.0.2 测试版（2026-09-13 进行中）：把已提交的 TASK-054~058 UI/主题改动升版本到 `1.0.2+3`，构建分 ABI 与通用 Release APK，打 tag `v1.0.2`，并在 Gitee 与 GitHub 发布同内容发行版。TASK-055 的视觉验收仍未关闭，本版按用户要求作为新的测试版分发；`v1.0.1` 发行版保持不变。
