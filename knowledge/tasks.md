...
- [x] TASK-050 发布 v1.0.1 测试版到 Gitee 发行版（2026-09-12 完成）：创建 release `1140059`（tag `v1.0.1`，标记预发布），上传通用包 `ncpu-timetable-1.0.1-universal.apk`（40,476,186 B）与 `app-arm64-v8a-release.apk`（22,184,832 B）。用公开接口读回确认附件可见，并实际下载通用包比对 SHA-256 与本地构建产物完全一致。发行版说明含两个包的 SHA-256、安装提示与已知限制。接口要点（`target_commitish` 必填、附件走 multipart）已记入 `knowledge/release.md`。令牌仅在本机命令行使用，未写入文件；**用户需自行撤销该令牌**（曾出现在聊天记录中）。该发行版随后由 TASK-051 重建为 `1140075`，下载地址未变。
