...
- [ ] TASK-055 重做 UI 为校园数字手账风格（2026-09-13 细节收口中）：整周课程标签已由 TASK-056 重构，并由 TASK-057 与详情页统一为同源深色渐变；用户不认可的背景网格已由 TASK-058 完全移除，并新增日间/夜间/跟随系统三档主题。`flutter analyze` 无问题、`flutter test` 135/135，Debug APK 构建安装成功，UIAutomator 语义完整，logcat 无致命异常或布局溢出。整体视觉仍等待用户继续验收，不提前标 Done。当前实际定稿方向是 TASK-062 的「校园线路图」，本任务的原始描述（手账风格）已被用户两次否定，待用户明确后应关闭或重写。

## Next

- [ ] TASK-039 Android 验收本地上课提醒（2026-09-13 模拟器部分完成，等待真机）：步骤见 `knowledge/notifications.md`。
  - 已完成（`ncpu_api36`，release 包，设备时区 `GMT`）：第 1–7 步全部取证——默认关闭、通知权限弹窗、精确闹钟不允许（非精确 `window=+1h`）与允许（精确 `window=0`）两个分支、**实际到点通知**（计划 05:30 UTC = 13:30 +08:00 到点收到，标题/正文正确、图标资源正确、点击不崩溃）、改课程与改提前量都精确重排、`adb reboot` 后开机接收器自动恢复、关闭提醒后待触发闹钟 196 → 0。
  - 期间发现并修复 2 个缺陷：ISSUE-017（release 资源裁剪删掉通知图标 → 开启提醒直接失败，`v1.0.0`～`v1.0.2` 三个已发布版本都受影响）、ISSUE-018（提醒时间基准用设备本地时间 + 单条过期提醒清空整批排程）。两处修复见 `knowledge/issues.md`。
  - 剩余（必须真机）：厂商省电/后台策略下的到点可靠性、厂商通知中心的展示与折叠。
...
- [x] TASK-063 发布 v1.0.3 测试版（2026-09-13 完成）：把 TASK-062 的「校园线路图」UI 与 ISSUE-017 / ISSUE-018 两个提醒修复升到 `1.0.3+4`（提交 `b1cc690`），构建分 ABI 三个包 + 通用包，打 tag `v1.0.3`（annotated `2e26bc862eaa`）。发布前检查：四个包逐个确认 `versionName=1.0.3` / 含 `INTERNET` / 正式证书 / **通知图标在包内**（ISSUE-017 的回归项）；模拟器覆盖安装冷启动 792 ms、非 debuggable、日志无致命异常与会话字段。发行版 GitHub `387810005` / Gitee `1140998`（均预发布，同一组附件），两个平台的通用包都实际下载比对 SHA-256 与本地一致（Gitee 30.3 s / GitHub 12.0 s），说明正文 492 汉字 / 0 U+FFFD。Gitee 令牌由用户提供、仅经环境变量传入脚本、未落盘，用毕需用户撤销。`UNVERIFIED`：arm64-v8a 真机安装（需先卸载、会丢本机数据，仍排在 TASK-019 / TASK-039 真机项之后）。
- [x] TASK-062 以「校园线路图」重构课表首页与全 App 卡片（2026-09-13 完成、已随 v1.0.3 发布）：用户否定 TASK-061 的校园编辑日历候选后，从交通线路图提炼站点语言——七日概览条、节次站点连线、详情页「一天十站」；并收口上一轮遗留的四处问题（移除径向环境光斑、空日列不再画整列卡片、今日卡与详情页迁移到新语言、切换按钮补足 48dp）。`flutter analyze` 无问题、`flutter test` 140/140，`ncpu_api36` 实画复核日间/夜间全部界面后覆盖安装验证。设计语言、遗留问题对照与证据见 `knowledge/report_2026-09-13_task062_route_language.md`。视觉是否最终认可仍由 TASK-055 跟踪。

- [x] TASK-061 重构整周视图与夜间首页（2026-09-13 技术完成、视觉否决）：候选版实现标题识别线、双曲轨道、环境光、`02` 周次编辑排版、半透明日列与柔光课程面；整周首屏约两天，当前周从今天开始循环排列。`flutter analyze`、136 项测试、Release 模拟器实画与日志技术验收均通过，但用户随后明确认为该版仍简陋、缺少艺术性，因此不作为视觉定稿，后续由 TASK-062 重新设计。

- [x] TASK-060 发布 v1.0.2 测试版（2026-09-13 完成）：把 TASK-054~058 的 UI/主题改动升到 `1.0.2+3`（提交 `9772985`），构建分 ABI 与通用 Release APK，打 tag `v1.0.2`，并走完发布前必查清单（`INTERNET`、正式证书、模拟器卸载 debug 后装 release、冷启动 1330 ms、教务登录页实测、日志无会话字段、日间/夜间实画）。`master` = `9265922`、tag `v1.0.2` 在两个远端读回一致。发行版两个仓库都建好：GitHub `387662208`、Gitee `1140399`（均预发布，同一组附件），两边的通用包实际下载 SHA-256 都与本地一致，说明文本均为 440 汉字 / 0 U+FFFD。Gitee 发行版因本机只有 10 位 git 口令而无 API 令牌，曾短暂 `BLOCKED`，由用户在对话中提供令牌后补建，用毕需撤销。TASK-055 的视觉验收仍未关闭，本版是测试版而非视觉定稿。
...
- [x] TASK-014 实现 Android V1 本地上课提醒（2026-09-12 完成）：默认关闭，用户主动开启时申请通知权限；默认提前 15 分钟，可选 5/10/15/30 分钟。按当前学期、课程周次/星期和统一作息生成未来提醒，课程、学期、作息、偏好变化及 App 回前台时重建；精确闹钟未授权时降级为非精确提醒；调度失败会回滚开启状态。`flutter analyze` 无问题、`flutter test` 130/130、Android JVM 单测 13/13、Debug APK 构建与通知权限/receiver/图标静态校验通过。真机行为转 TASK-039。
  ⚠️ **后续更正（2026-09-13，ISSUE-017）**：本轮“图标静态校验通过”只能证明文件存在，无法发现 release 资源裁剪会把通知图标删掉——该缺陷使 `v1.0.0`～`v1.0.2` 的 release 包开启提醒直接失败。已于 TASK-039 修复并装机验证。
