...
- [ ] TASK-062 以跨品类设计理念重构课表首页（2026-09-13 进行中，已过模拟器技术验收）：用户明确否定 TASK-061 的校园编辑日历候选，要求不局限于课表 App。当前方向从交通线路图、登机牌与音乐播放器提炼“校园线路图”：整周七天概览始终在首屏可见，课程按时间作为线路站点展开；移除大号周次数字、环境光斑、巨型空日卡和卡片套卡片。当前周仍须从当天开始，点击任一天可直接定位；日间/夜间、满课一屏、末节不遮挡与 48dp 触控目标继续作为验收约束。
  - 已实现并应用户要求通过模拟器验证（2026-09-13，`ncpu_api36`）：`analyze` 无问题、140 项测试通过；用当前代码构建正式签名 Release 并 `install -r` 覆盖 v1.0.2（保留本机 13 门课程），冷启动 760 ms，日间/夜间首页、整周、详情、设置、新增表单、路线图点击定位、周切换、横滑与增删课程均正常，logcat 无致命异常或布局溢出。细节见 `knowledge/changelog.md` 与 `knowledge/testing.md`。
  - **上一轮遗留问题已全部修完**（2026-09-13 收口）：①移除径向环境光斑，背景只留渐变与线路水印；②空日列不再画整列卡片，只留两条极淡列轨并把「沿线无课」移到线路上端；③「今日」列表卡与课程详情页迁移到新语言（新增 `courseSurfaceTint()` / `courseBadgeTint()` 统一课程色用法，详情页另加「一天十站」节次线路），设置页身份卡同步改为中性表面 + 蓝色焦点，海军蓝药丸与亮黄节次章已从 App 内清除；④「今日/整周」切换按钮由约 41dp 改为固定 48dp。
  - **剩余**：OnePlus 真机未安装本候选包；改动仍在工作区，是否出包/提交待用户决定。`UNVERIFIED`：真机视觉认可，由 TASK-055 跟踪。
