...
- **小组件**：载荷 `schemaVersion` 升到 2，增加 `sections[].group` 与 `schoolId`。Dart 与 Kotlin 双实现必须同步改并两侧都加测试（DEC-009 的既有风险）。**推荐做法**：由 Dart 预先算出整个学期的"每天有哪些课"再推给原生，原生只按日期查表 —— 这样既消除双实现的一致性风险，又保留"App 未运行也正确"（原生不再需要自己推周次）。代价是载荷变大，需实测。→ 见 [D6](#d6-桌面小组件的周次计算怎么处理)。
...
每个决策都给三样东西：**选项**、**你得到什么**、**代价**。推荐项排在最前并标注。

### D1 一台手机上同时装几所学校的课表？

| 选项 | 具体是什么 | 你得到什么 | 代价 |
| --- | --- | --- | --- |
| **A. 单校激活（推荐）** | 装好后选一所学校，课表、小组件、提醒都只服务这一所；换学校时旧数据留着但不显示 | 实现简单，改动集中在"加一层配置"；与"本地单机、不做账号"的定位一致 | 想同时看两所学校的课表做不到 |
| B. 多校共存 | 同一设备同时显示多所学校的课表，能切换查看 | 一个 App 管全部 | 数据库、学期、提醒、小组件都要多一个"学校"维度，工作量大致翻倍；小组件还要决定显示哪一所 |

影响：决定阶段 1 的数据库迁移范围。选 A 时 `schoolId` 仍会加上（为将来留门），只是不做多校 UI。

### D2 各校的校历和作息时间表从哪里来？

背景：学校每年/每学期会调整作息（节次时间）和教学周历，App 必须能跟上，否则周次和提醒都会错。

| 选项 | 具体是什么 | 你得到什么 | 代价 |
| --- | --- | --- | --- |
| **A. 内置 + 用户在 App 里改（推荐）** | 已知学校把校方公布的作息/周历内置；用户在设置页可以直接改每节时间和第一周周一 | 学校调整后用户自己改一下就行，不用等发版；实现最直接 | 用户要自己知道学校的新时间（通常看教务处公告） |
| B. 内置 + 用户导入配置文件 | 同上，但改法是把一个 JSON/文本文件导进 App | 适合一次改很多项，或别人分享配置 | 多一层文件格式与导入 UI；对普通用户比 A 更绕 |
| C. 内置 + App 自动联网下载 | App 从网上拉最新校历配置 | 用户完全不用管 | App 多了一个网络依赖；还要解决"配置由谁维护、被改坏了怎么办"（需要签名校验）；与"不做后端"的定位冲突 |

影响：决定阶段 2 做多少。A 和 B 不冲突，可以先做 A；C 只在阶段 4 评估，且需要先有稳定的配置维护方。

### D3 新学校如果只有 HTTP（不是 https）入口，能接受"加一所就要重新发一次版"吗？

背景：Android 和 iOS 都要求在**安装包里**声明哪些域名允许明文 HTTP。装到手机之后没法再打开这个开关，所以新增一家 HTTP 学校必须改配置重新打包。HTTPS 学校不受此限，可以直接接入。

| 选项 | 你得到什么 | 代价 |
| --- | --- | --- |
| **A. 接受（推荐）** | HTTP 学校也能接（很多学校教务还在明文 HTTP 上） | 每加一所 HTTP 学校要发一次新版；旧版用户要么升级，要么用不了这所学校 |
| B. 不接受，只接 HTTPS 学校 | 新学校随时可加，不用发版 | 能接的学校范围明显变小 |

影响：决定阶段 3 选校的可行范围。

### D4 阶段 3 用哪所真实学校来验收？

阶段 3 必须有一所真实学校才能证明架构真的通用（合成数据只能验证代码能跑，不能验证"另一个教务系统真的长这样"）。需要的前提：**有人有该学校的账号，并愿意在 Debug 构建里自己登录一次**，用于脱敏采集接口形状（不采集密码，不落盘身份字段）。

| 选项 | 含义 |
| --- | --- |
| 现在就有目标学校 | 请在回复里写明校名和教务入口地址，我把采集步骤准备好 |
| 暂时没有 | 阶段 0–2 照常做（它们不依赖具体学校），阶段 3 挂起等学校 |

### D5 调休、停课、考试周要不要早点支持？

背景：现在"第 N 周"是从第一周周一往下数的连续 7 天。国庆调休、校运会停课那几周，课表和提醒都会照着正常周算错。

| 选项 | 你得到什么 | 代价 |
| --- | --- | --- |
| A. 先不做，只明确提示"本校调休未支持" | 省事，不会静默出错 | 调休那几周的课表和提醒确实是错的，用户得自己注意 |
| **B. 阶段 2 就让用户能手动录入例外（推荐，若目标学校调休多）** | 那几周能改对 | 多一个录入界面；用户每年要手动补几次 |

影响：主要看 D4 选的学校调休多不多。

### D6 桌面小组件的周次计算怎么处理？

背景：现在"今天是第几周、今天上什么课"在 Dart 和 Kotlin 里**各写了一套**。校历一改就要同时改两个地方，漏一个就会出现"App 显示第 3 周、小组件显示第 2 周"。

| 选项 | 你得到什么 | 代价 |
| --- | --- | --- |
| **A. Dart 预先算好整个学期的每天课程，原生只按日期查表（推荐）** | 彻底消除双实现风险，不会再出现两侧不一致；小组件在 App 未运行时仍然正确 | 载荷变大（估算几十 KB 量级，需实测）；换学期/改校历后要重新推送一次 |
| B. 保留两套，但把校历参数（第一周周一、总周数、节次分组）全部数据化后推给原生 | 改动最小；原生只剩一个简单的"日期→第几周"公式 | 仍然是两份代码，校历语义一变还是有同步遗漏的风险 |
| C. 维持现状 | 不动现在能跑的东西 | 校历改动时的不一致风险一直存在 |

影响：与阶段 1 的展示层改造一起做最省事，因为那时本来就要动小组件载荷（`schemaVersion` 升到 2）。**推荐 A。**
...
| --- | --- | --- |
| D1 | 单校激活 / 多校共存 | 待定 |
| D2 | 校历作息数据来源（A / B / C） | 待定 |
| D3 | 是否接受 HTTP 学校需重新发版 | 待定 |
| D4 | 阶段 3 的目标学校 | 待定 |
| D5 | 调休是否提前到阶段 2 | 待定 |
| D6 | 小组件周次计算方案（A / B / C） | 待定 |
